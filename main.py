...
# --- Множество активных потоков: ключ = (chat_id, url) ---
active_monitors = {}
...
def monitor_link(chat_id, url):
   ...
   while True:
       # Проверяем существование ссылки в БД
       try:
           with get_db() as conn:
               cur = conn.cursor()
       ...
       # Читаем прошлый статус и настройки пользователя
       try:
           with get_db() as conn:
               cur = conn.cursor()
               cur.execute("""
...
def test_apple_connection(message):
   ...
   try:
       ...
       if status_code in (403, 429):
           bot.reply_to(message, f"🚫 Код: {status_code}. Apple заблокировал IP сервера.")
